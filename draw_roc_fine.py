...
if __name__ == "__main__":
    ...
    mode = 'test'
    weight_path = '/data2/ceiling/workspace/RCFNet/save_model/ROI_ambiguous_RCF/net2_BestAcc.pth'
    save_prob = '/data2/ceiling/workspace/RCFNet/save_model/ROI_ambiguous_RCF/prob_fine_{}.csv'.format(mode)
    model = FineNet()
    save_path = '/data2/ceiling/workspace/RCFNet/save_model/ROI_ambiguous_RCF/ROC_fine_{}.png'.format(mode)
    title = 'ROC of FineNet'
    ...
    for step, pack in enumerate(test_loader, start=1):
        ...
        csv_rows.append([step, name[:-4], label, torch.argmax(r, dim=-1).cpu().numpy()[0], pred[0], pred[1], pred[2], pred[3], pred[4], ])
