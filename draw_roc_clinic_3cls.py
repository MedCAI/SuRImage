...
if __name__ == "__main__":
    ...
    weight_model_path = '/data2/ceiling/workspace/RCFNet/save_model/ROI_303_Fusion_clinic_RCFNet_5cls_20240206/fusion_epoch_3.pth'
    save_path = '/data2/ceiling/workspace/RCFNet/save_model/ROI_303_Fusion_clinic_RCFNet_5cls_20240206/ROC_clinic_{}.png'.format('3cls')
    ...
    save_prob = '/data2/ceiling/workspace/RCFNet/save_model/ROI_303_Fusion_clinic_RCFNet_5cls_20240206/prob_clinic_{}.csv'.format('3cls')
    ...
    y_true = []
    y_scores = []
    csv_header = ['step', 'patient name', 'label', 'AIS', 'MIA', 'IAC']
    csv_rows = []
    ...
    for step, pack in enumerate(test_loader, start=1):
        ...
        f = torch.cat([f[:, 0:1], f[:, 1:2], f[:, 2:3] +  f[:, 3:4] + f[:, 4:5]], dim=1)
        label = label.cpu().numpy()[0]
        pred = f.cpu().numpy()[0]
        name = name[0]
        y_true.append(label)
        y_scores.append(pred)
        csv_rows.append([step, name[:-4], label, pred[0], pred[1], pred[2]])
